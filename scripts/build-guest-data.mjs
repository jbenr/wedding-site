...
function withCount(members, guestsOnInvite, lastName, placeholderLabel) {
...
function parseInvite(lastName, invite, guestsOnInvite) {
  ...
  if (/^the\s+.+\s+family$/i.test(text)) {
    return {
      members: withCount([], guestsOnInvite, lastName, "unnamed family — fill in real names"),
      unparsed: true
    };
  }
  ...
  if (andGuest) {
    ...
    return { members: withCount([named], guestsOnInvite, lastName, "unnamed +1") };
  }
  ...
  if (twoNamed) {
    ...
    return { members: withCount([leftNamed, rightNamed], guestsOnInvite, rightNamed.lastName || lastName, "unnamed extra") };
  }
  ...
  if (twoFirstNamesSharedLast) {
    ...
    return { members: withCount(named, guestsOnInvite, sharedLast, "unnamed family") };
  }
  ...
  if (mrAndMrs) {
    ...
    return { members: withCount([named], guestsOnInvite, named.lastName || lastName, "unnamed spouse/family") };
  }
  ...
  if (single) {
    ...
    return { members: withCount([named], guestsOnInvite, named.lastName || lastName, "unnamed extra") };
  }

  // Nothing matched — bail out with a flagged placeholder.
  return {
    members: withCount([], guestsOnInvite, lastName, `could not parse invite line: "${invite}"`),
    unparsed: true
  };
}
...
for (const [lastName, invite, guestsOnInvite, rehearsal] of RAW_ROWS) {
  ...
  households.push({
    id,
    members: members.map(({ placeholder, note, ...m }) => m),
    events
  });
